...
const Comments = ({post}) => {
  ...
  useEffect(()=>{
    const newCmt = post.comments.filter(cm => !cm.reply)
    setComments(newCmt)
    ...
    <div className='comments'>
      {
        showComments.map((comment, index) => (
          <CommentDisplay key={index} comment={comment} post={post} replyCmts={replyComments.filter(item => item.reply === comment._id)} />
        ))
      }
      {
